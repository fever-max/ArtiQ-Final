...
import '../../../../styles/main/userInfo/myPage/MyCommunity.css';
...
function MyCommunity() {
  ...
  return (
    <div className="MyCommunity_div">
      <div className="MyCommunity_div">
        <h3 className="MyCommunity_title">커뮤니티 게시글</h3>
        ...
        <div className="MyCommunity_Table">
          ...
          <div className="MyCommunity_table_column">
            <div className="MyCommunity_table_column1">카테고리</div>
            <div className="MyCommunity_table_column2">제목</div>
            <div className="MyCommunity_table_column3">작성일</div>
            <div className="MyCommunity_table_column4">조회수</div>
            <div className="MyCommunity_table_column5">삭제</div>
          </div>
          <hr className="MyCommunity_hr" />
          {boardFreeData.length === 0 ? (
            <div className="MyPageSub_noDataMessage">작성한 자유게시글이 없습니다.</div>
          ) : (
            boardFreeData.map((board, index) => (
              <div className="MyCommunity_table_column_sub" key={index}>
                <div className="MyCommunity_table_column1">{board.boardCategory}</div>
                <div className="MyCommunity_table_column2">
                  <Link to={`/freeBoardDetail/${board.boardNumber}`} className="MyCommunity_table_column_txt">
                    {board.boardTitle}
                  </Link>
                </div>
                <div className="MyCommunity_table_column3">{board.boardDate}</div>
                <div className="MyCommunity_table_column4">{board.boardViewCount}</div>
          ...
          {boardQuestionData.length === 0 ? (
            ...
            boardQuestionData.map((board, index) => (
              <div className="MyCommunity_table_column_sub" key={index}>
                ...
                <div className="MyCommunity_table_column4">{board.questionViewCount}</div>
                <div className="MyCommunity_table_column5">
